...
class CursoController extends Controller
{
    public  function index(){
        ...
        //return "Hola desde index de CursoController. Esto sale al poner /cursos por GET";
        //return view('cursos/index');

        //$cursos = Curso::all(); //todos los cursos de la tabla cursos de la base de datos.
        //return $cursos; //para pruebas.
        $cursos = Curso::orderby('id','desc')->paginate(10); //pagina de 15 en 15 por defecto. Poner $cursos->links(); para enlaces en vista cursos.index
    ...
    public  function show(Curso $curso){ ////si el parámetro fuera simplemente $curso, tendríamos que hacer el find
    ...
    public  function edit(Curso $curso){ /////si el parámetro fuera simplemente $id, tendríamos que hacer el find para variable $curso

        //$curso = Curso::find($id); //para pasar el modelo objeto completo y no sólo su nombre o id.
        return view('cursos.edit', compact('curso'));
    }

    public  function update(Curso $curso, Request $request){ ///OJOOOOOO la variable TIENE QUE llamarse $curso para poder recibirla así
        //ACTUALIZAMOS REGISTRO, validando primero. Se puede hacer con FormRequest como en el store()
        $request->validate([ //son los campos del formulario de la vista cursos.edit
            'name' => 'required',
            'description' => 'required',
            'categoria'=> 'required'
        ]);
        ...
        //return $curso;
        return view('cursos.show', ['curso' => $curso]); //en lugar de compact('curso').
    }
